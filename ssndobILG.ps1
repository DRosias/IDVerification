...
$zHRDepartmentID = get-aduser -Server "ad.mvwcorp.com" -identity $user -Properties zHRDepartmentID | Select -expand zHRDepartmentID
$zHRLocationCode = get-aduser -Server "ad.mvwcorp.com" -identity $user -Properties zHRLocationCode | Select -expand zHRLocationCode

#pull Manager info. If no manager info, set message
Try{
...
#pull logon time and make friendly. If never logged on, set message
Try{
...
if($zHRDepartmentID -eq $null){write-host "                          zHRDepartmentID Attribute is invalid `r`n"}
if($zHRLocationCode -eq $null){write-host "                          zHRLocationCode Attribute is invalid `r`n"}
}
...
if ($mvw){
	...
	#try to check if user is vri.ilg.ad. If not, catch to next domain.
		...
		Catch{
			#cls and rewrite input for a consistent display
            cls
